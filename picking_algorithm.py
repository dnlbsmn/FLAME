...
def image_receive(image, system):
    ...
    if arm.pick_state == 0:
        ...
        if kiwifruits != []:
            ...
            kiwifruit = arm.nerian_to_position(kiwifruits[0])
    ...
    arm.send_state(arm.pick_state)

# Start the thread to record the TCP poses
def record_tcp(arm):
    # Update the UR5 TCP pose constantly
    while arm.pick_state != 5:
        arm.receive_tcp_pose()
        time.sleep(0.01)

    # Exit if the end state has been called
    return
